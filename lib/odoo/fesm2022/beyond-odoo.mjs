...
import { InjectionToken, inject, Injectable, EventEmitter, signal, computed, Output, Input, Component, Inject } from '@angular/core';
...
class OdooJsonConnector {
    ...
    login$(user, password) {
        ...
        if (!user) {
            ...
            return this.searchCount$('res.users', [['id', '=', 2]]).pipe(map(() => 2), catchError(err => { this.permissionsServices.reset(); return of(err.message); }));
        }
...
class BydMessagesService extends BydBaseOdooService {
    ...
    async postMessage$(id, message, files) {
        ...
        return (attachments.length > 0 ? forkJoin([...attachments.map(attachment => this._odooService.create$('ir.attachment', { ...attachment, ...{ res_id: id } }))]) : of([])).pipe(mergeMap((ids) => {
...
class BydAttachementsService extends BydBaseOdooService {
    ...
    fetch$(id, model) {
        return this.attachments.fetch(this.key(id, model), this._odooService.searchRead$('ir.attachment', [
            ['res_id', '=', id],
    ...
    async post$(id, model, files) {
        ...
        return (attachments.length > 0 ? forkJoin([...attachments.map(attachment => this._odooService.create$('ir.attachment', { ...attachment, ...{ res_id: id } }))]) : of([]));
    }
...
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "19.2.1", ngImport: i0, type: ScanPackingDialog, decorators: [{
            type: Component,
            args: [{ selector: '', standalone: true, imports: [LoaderComponent, EmptyComponent, ErrorComponent, ZXingScannerModule, CardComponent, CardHeaderComponent, CardTitleComponent, TranslatePipe, MatIcon, CardCtaComponent], template: "<div class=\"p-space-md\">\r\n  @if (this.step() === 'scan') {\r\n    <zxing-scanner\r\n      (scanSuccess)=\"this.scanSuccess($event)\"\r\n      (permissionResponse)=\"this.permissionResponse($event)\"\r\n    ></zxing-scanner>\r\n  } @else if (this.step() === 'search') {\r\n    <byd-loader [isLoading]=\"this.requestState.isLoading()\">\r\n      <byd-error [message]=\"this.requestState.getErrorMessage()\" [code]=\"this.requestState.getErrorStatus()\">\r\n        <byd-empty [isEmpty]=\"this.noData\" [isLight]=\"false\">\r\n          <div class=\"grid\">\r\n            @for (scope of this.scopes; track scope) {\r\n              @if (this.getDataByScope(scope).length > 0) {\r\n                <div>\r\n                  <byd-card [highlight]=\"this.activeScope?.key === scope.key\" (click)=\"this.setScope(scope)\">\r\n                    <byd-card-header class=\"ta-c\">\r\n                      <byd-card-title>\r\n                        {{ 'scan.' + scope.key + '.name' | translate }} ({{ this.getDataByScope(scope).length }})\r\n                      </byd-card-title>\r\n                    </byd-card-header>\r\n                  </byd-card>\r\n                </div>\r\n              }\r\n            }\r\n          </div>\r\n          @if (this.activeScope) {\r\n            <div>\r\n              @for (item of this.getDataByScope(this.activeScope); track item) {\r\n                <div class=\"flex-column justify-content-between m-space-md\">\r\n                  <byd-card (click)=\"this.navigateTo(this.activeScope, item)\">\r\n                    <byd-card-header class=\"ta-c\">\r\n                      <byd-card-title>\r\n                        {{ item.name }}\r\n                      </byd-card-title>\r\n                    </byd-card-header>\r\n                    <byd-card-cta>\r\n                      <mat-icon>chevron_right</mat-icon>\r\n                    </byd-card-cta>\r\n                  </byd-card>\r\n                </div>\r\n              }\r\n            </div>\r\n          }\r\n        </byd-empty>\r\n      </byd-error>\r\n    </byd-loader>\r\n  }\r\n</div>\r\n" }]
...
export { BydAttachementsService, BydAuthOdooService, BydBaseOdooService, BydEmployeeService, BydMessagesComponent, BydMessagesService, BydPrinterService, BydScanPackingComponent, BydScanPackingService, BydUploadComponent, ContainerComponent, ODOO_SERVER_CONFIG_KEY, TargetUrlInterceptor, provideOdoo };
