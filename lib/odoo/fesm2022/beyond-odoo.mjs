...
import { filter as filter$1, map as map$1, mergeMap as mergeMap$1, switchMap, tap as tap$1 } from 'rxjs/operators';

import { BydFormComponent } from '@beyond/form-basic';
import { TextBoxComponent } from '@beyond/form-input';
import { InputCheckBox, InputDropdown, InputLabel, InputNumber, InputPanel, InputTextBox } from '@beyond/form-model';
import { BydNotificationService, ENotificationCode } from '@beyond/notification';
import { BydPermissionsServices, HandleComplexRequest } from '@beyond/server';
...
import { BehaviorSubject, Subject, catchError, filter, forkJoin, map, mergeMap, of, tap } from 'rxjs';
      ...
      attachments.length > 0
        ? forkJoin([
            ...attachments.map(attachment =>
              this._odooService.create$('ir.attachment', { ...attachment, ...{ res_id: id } })
            ),
          ])
        : of([])
    ).pipe(
      mergeMap(ids => {
        return this._odooService.create$('mail.message', { ...message, ...{ subtype_id: 2, attachment_ids: ids } });
      })
    );
  }
  static ɵfac = i0.ɵɵngDeclareFactory({
...
class BydUploadComponent {
  ...
  static ɵcmp = i0.ɵɵngDeclareComponent({
    ...
    inputs: { features: 'features', canSelectMultipleFiles: 'canSelectMultipleFiles' },
    outputs: { filesPicked: 'filesPicked' },
    ngImport: i0,
    template:
    ...
    dependencies: [
      {
        kind: 'component',
        type: BydButtonComponent,
        selector: 'byd-button',
        inputs: ['state', 'type', 'size', 'icon', 'options', 'stopPropagationActivation'],
        outputs: ['action'],
      },
      {
        kind: 'component',
      ...
      { kind: 'pipe', type: TranslatePipe, name: 'translate' },
    ],
  });
}
i0.ɵɵngDeclareClassMetadata({
      ...
      args: [
        {
          ...
          template:
            '<div class="grid align-items-center mb-space-xs">\r\n  @for (pic of this.tempImages(); track pic.localUrl) {\r\n    <div class="temp-pic-container g-col-3">\r\n      <img [src]="pic.localUrl" width="100%" />\r\n      <div class="remove-cta" (click)="this.remove(pic)">\r\n        <mat-icon>close</mat-icon>\r\n      </div>\r\n    </div>\r\n  }\r\n  <div>\r\n    @if (this.addActions.length === 1) {\r\n      <byd-button (action)="this.addActions[0].callback()">\r\n        <mat-icon>{{ this.addActions[0].icon | translate }}</mat-icon>\r\n      </byd-button>\r\n    } @else if (this.addActions.length > 1) {\r\n      <byd-button *ngFor="let action of this.addActions" (action)="action.callback()">\r\n        <mat-icon>{{ action.icon }}</mat-icon>\r\n      </byd-button>\r\n    }\r\n  </div>\r\n</div>\r\n',
          styles: [
            '.temp-pic-container{position:relative;padding:2px}.remove-cta{position:absolute;top:0;right:2px}\n',
          ],
        },
      ],
    },
...
class BydMessagesComponent extends BydBaseComponent {
  ...
  static ɵcmp = i0.ɵɵngDeclareComponent({
    ...
    styles: [
      '.item-message{margin:5px 0;padding:5px;border:1px solid var(--byd-surface-brand-primary);border-radius:5px}\n',
    ],
    dependencies: [
      {
        kind: 'component',
      ...
      { kind: 'component', type: ErrorComponent, selector: 'byd-error', inputs: ['message', 'code'] },
      {
        kind: 'component',
        type: TextBoxComponent,
        selector: 'byd-input-textbox',
        inputs: ['input', 'matcher', 'space'],
      ...
      },
      {
        ...
        type: BydUploadComponent,
        selector: 'byd-files-upload',
        inputs: ['features', 'canSelectMultipleFiles'],
        outputs: ['filesPicked'],
      },
  ...
  static ɵcmp = i0.ɵɵngDeclareComponent({
    ...
    dependencies: [
      { kind: 'component', type: LoaderComponent, selector: 'byd-loader', inputs: ['isLoading', 'skeleton'] },
      {
        ...
        type: EmptyComponent,
        selector: 'byd-empty',
        inputs: ['isEmpty', 'isLight', 'showMessage', 'text', 'type', 'icon', 'iconSize'],
      },
      { kind: 'component', type: ErrorComponent, selector: 'byd-error', inputs: ['message', 'code'] },
      { kind: 'ngmodule', type: ZXingScannerModule },
      {
        kind: 'component',
        type: i2.ZXingScannerComponent,
      ...
      },
      {
        ...
        type: CardComponent,
        selector: 'byd-card',
        inputs: ['highlight', 'shadow', 'fullHeight', 'noContent', 'isNew'],
        outputs: ['click'],
      },
      { kind: 'component', type: CardHeaderComponent, selector: 'byd-card-header' },
      { kind: 'component', type: CardTitleComponent, selector: 'byd-card-title' },
      { kind: 'pipe', type: TranslatePipe, name: 'translate' },
      {
        kind: 'component',
        type: MatIcon,
      ...
      { kind: 'component', type: CardCtaComponent, selector: 'byd-card-cta' },
    ],
  });
}
i0.ɵɵngDeclareClassMetadata({
...
class BydPrinterService extends BydBaseOdooService {
  ...
  getPrintWizard$(id) {
    return this._printWizard$.pipe(
      map$1(data => data[id]),
      filter$1(myData => !!myData)
    );
  }
  printWizard$(data) {
    const model =
      data.model === 'print.direct.wizard'
        ? this._odooService.searchRead$(
            'print.direct.wizard',
            [['id', '=', data.id]],
...
class LabelModal extends BydBaseComponent {
  ...
  static ɵcmp = i0.ɵɵngDeclareComponent({
    ...
    dependencies: [
      { kind: 'component', type: LoaderComponent, selector: 'byd-loader', inputs: ['isLoading', 'skeleton'] },
      { kind: 'component', type: ErrorComponent, selector: 'byd-error', inputs: ['message', 'code'] },
      { kind: 'component', type: BydTitleComponent, selector: 'byd-title', inputs: ['level', 'isTheme', 'isBold'] },
      {
        kind: 'component',
        type: BydFormComponent,
        selector: 'byd-form',
        inputs: [
          'inputs',
          'askValidation$',
...
export {
  BydAttachementsService,
  BydAuthOdooService,
  BydBaseOdooService,
  BydEmployeeService,
  BydMessagesComponent,
  BydMessagesService,
  BydPrinterService,
  BydScanPackingComponent,
  BydScanPackingService,
  BydUploadComponent,
  ContainerComponent,
  ODOO_SERVER_CONFIG_KEY,
  TargetUrlInterceptor,
  provideOdoo,
};
