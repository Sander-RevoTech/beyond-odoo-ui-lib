...
import { tap, map, Subject, BehaviorSubject, filter, mergeMap, forkJoin } from 'rxjs';
...
class AppMessagesService extends BydBaseOdooService {
    ...
    async postMessage$(id, message, files) {
        ...
        return this._odooService.create$('mail.message', { ...message, ...{ subtype_id: 2 } })
            .pipe(filter(data => !!data), mergeMap(data => forkJoin([...attachments.map(attachment => this._odooService.create$('ir.attachment', { ...attachment, ...{ res_id: data.id } }))])));
    }
