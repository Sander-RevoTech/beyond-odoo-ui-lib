...
import { BydBaseComponent, BydAbstractComponent, isNonNullable } from '@beyond/utils';
...
import { mergeMap, of, filter, map as map$1, tap } from 'rxjs';
...
class BydUserService extends BydBaseOdooService {
    ...
    fetchProfile$() {
        ...
        return this.profile$.fetch(this._odooService
            .searchRead$('res.users', [['id', '=', this.permissionsServices.uid]], ['id', 'email', 'display_name', 'share', 'groups_id'])
            .pipe(filter(isNonNullable), map$1((result) => result[0]), tap((profile) => {
